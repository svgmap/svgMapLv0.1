...
(function (window, undefined) {
	...
	var svgMapGIStool = (function () {
		...
		function getIncludedPointsS2(geom, superParam) {
			...
			for (
				var poiDocC = 0;
				poiDocC < pointsDocTreeIDs.length;
				poiDocC++
			) {
		...
		function getIncludedPointsS2a(geom, superParam) {
			...
			var polygonsDocTreeIDs = getChildDocsId(
				superParam.polygonsDocTreeID
			);
	...
	**/
		...
		**/
		...
		function buildDifferenceS2(
			...
			loop1: for (var i = loop1Count; i < fa1.length; i++) {
				if (j == 0) {
					...
					try {
						featureA =
							jsts.simplify.DouglasPeuckerSimplifier.simplify(
								featureA,
								0.00001
							);
						featureA =
		...
		function buildIntersectionS2a(geom, params) {
			...
			var jstsFeature1 = getFeature(
				buildGeoJsonGeometryCollectionFromGeometryArray(fa1)
			);
			var jstsFeature2 = getFeature(
			...
			jstsFeature1 = jsts.simplify.DouglasPeuckerSimplifier.simplify(
				jstsFeature1,
				0.00001
			);
			jstsFeature2 = jsts.simplify.DouglasPeuckerSimplifier.simplify(
				jstsFeature2,
				0.00001
			);
			console.log(
			...
			jstsFeature1 = gpr.reduce(
				jstsFeature1,
				new jsts.geom.PrecisionModel(1000000)
			);
		...
		function buildGeoJsonFeatureCollectionFromGeometryArray(geometryArray) {
			// not used
			var fc = {};
		...
		function buildGeoJsonGeometryCollectionFromGeometryArray(
			geometryArray
		) {
			var gc = {};
...
//			console.log("layerId:",docId , " rootLayer:",svgImagesProps[docId].rootLayer);
		...
		**/
		...
		function geomHasCoordinates(geom) {
		...
		function buildIntersectionS3(
			...
			while (counter < compArray.length && halt == false) {
				...
				if (m == 0) {
					...
					feature2 = jsts.simplify.DouglasPeuckerSimplifier.simplify(
						feature2,
						0.00001
					);
				}
					...
					if (isf) {
						...
						if (geomHasCoordinates(isGeom)) {
							if (params.addMetadata) {
								...
								var meta1 =
									geom[docIdA][geomIndexA].src.getAttribute(
										"content"
									); // 重ければ一括前処理で効率化できる
								var meta2 =
									geoms2[l].src.getAttribute("content"); // 同上
			...
			if (counter == compArray.length) {
				...
				console.log(
					"svgmapGIS:geoJsonIntersections",
					geoJsonIntersections
				);
				if (params.getResultAsGeoJsonCallback) {
					// 2020/1/8
		...
		function getInRangePolygonParts(
			poiID_or_points,
			coverID,
			rangeData,
			cbFunc,
		...
		function getInRangeGeometriesOnCoverage(
			poiID_or_points,
			coverID,
			rangeData,
			cbFunc,
			...
			for (var i = 0; i < poiGeoms.length; i++) {
				if (poiGeoms[i].type.toLowerCase() == "point") {
					...
					for (var j = 0; j < poiGeoms[i].coordinates.length; j++) {
						for (
							var k = 0;
							k < poiGeoms[i].coordinates[j].length;
							k++
						) {
		...
		function getInRangePointsS2(geom, superParam) {
			...
			for (var layerId in geom) {
				if (
					...
					svgImagesProps[layerId].rootLayer ==
						superParam.pointsDocTreeID
				) {
					for (var i = 0; i < geom[layerId].length; i++) {
						if (superParam.computingOptions.targetVectorType == 0) {
							...
							superParam.computingOptions.targetVectorType == 1
						) {
							...
							superParam.computingOptions.targetVectorType == 2
						) {
			...
			superParam.coverageIndex = getNextIntersectedCoverage(
		...
		function getNextIntersectedCoverage(
			targetCoverages,
			targetGeomsBbox,
			currentIndex
		) {
		...
		function computeInRangePoints(
			...
			for (var i = 0; i < superParam.targetGeoms.length; i++) {
				...
				if (superParam.computingOptions.targetVectorType == 1) {
					...
					if (
						...
						var canXY = latLng2coverageImageXY(
							pixWidth,
							pixHeight,
							geomCrd[0],
							geomCrd[1],
						...
						if (judgeVal((100 * A) / 255, superParam.range.alpha)) {
							...
							if (
								judgeVal(hsv.h, superParam.range.hue, true) &&
								judgeVal(hsv.s, superParam.range.satulation) &&
								judgeVal(hsv.v, superParam.range.value)
							) {
								hsv.a = A;
								hsv.r = R;
								hsv.g = G;
			...
			if (superParam.progrssCallback) {
				superParam.progrssCallback(
					Math.ceil(
						100 *
							(superParam.coverageIndex /
								superParam.targetCoverages.length)
					)
				);
			}
			...
			if (
				...
				console.log(
					...
					"   comp.Ratio[%]:",
					Math.floor(
						(10000 * computingOverhead) /
							(computingOverhead + fileGetOverhead)
					) / 100
				);
		...
		function getImagePixData(
			...
			if (imageCacheEnabled && imageCache[imageURL_int]) {
				...
				if (
					...
					getImageURL(imgSrcURL) == imgSrcURL
				) {
		...
		function returnImageRanderedCanvas(
			img,
			callbackFunc,
			callbackFuncParams
		) {
			...
			var pixData = cContext.getImageData(
				0,
				0,
				canvas.width,
				canvas.height
		...
		function computeInRangeLineParts(
			pixData,
			pixWidth,
			pixHeight,
			geomCrd,
			...
			if (svgMap.isIntersect(lExt, cExt)) {
				...
				for (var i = 0; i < cline.length - 1; i++) {
					...
					if (cline[i + 1].clippedEdge != true) {
						...
						var startXY = latLng2coverageImageXY(
							pixWidth,
							pixHeight,
							startP.x,
							startP.y,
						...
						var endXY = latLng2coverageImageXY(
							pixWidth,
							pixHeight,
							endP.x,
							endP.y,
						...
						var pixs = getStraightLineCoordinates(
							[startXY.x, startXY.y],
							[endXY.x, endXY.y]
						);
						...
						for (var j = 0; j < pixs.length; j++) {
							...
							var geoPos = coverageImageXY2LatLng(
								pixWidth,
								pixHeight,
								pixs[j][0],
								pixs[j][1],
							...
							if (comPix.inBounds) {
								if (
									superParam.computingOptions
										.splitByCoverage == true
								) {
									if (
										!superParam.ans[
											superParam.coverageIndex
		...
		function computeInRangePolygonParts(
			pixData,
			pixWidth,
			pixHeight,
			geomCrd,
			...
			if (svgMap.isIntersect(lExt, cExt)) {
				...
				for (var i = 0; i < geomCrd.length; i++) {
					var canXY = latLng2coverageImageRawXY(
						pixWidth,
						pixHeight,
						geomCrd[i][0],
						geomCrd[i][1],
				...
				if (!superParam.ans[superParam.coverageIndex]) {
					...
					superParam.ans[superParam.coverageIndex].rasterData =
						rasterData;
					superParam.ans[
						superParam.coverageIndex
					].hasIntersection = false; // そのタイル内に、ポリゴンが被っている
					superParam.ans[superParam.coverageIndex].hasInRange = false; // そのタイルにポリゴンが被っており、なおかつレンジ内がある
					superParam.ans[
						superParam.coverageIndex
					].hasOutOfRange = false; // そのタイルにポリゴンが被っており、なおかつレンジ外がある
				...
				rasterData =
					superParam.ans[superParam.coverageIndex].rasterData; // 実際の演算結果のsuperParam.ans[].rasterDataへの紐づけ
				...
				superParam.ans[superParam.coverageIndex].inRangeCounts =
					inRangeCounts;
		...
		function computePixCoverage(
			...
			if (judgeVal((100 * A) / 255, range.alpha)) {
				...
				if (
					judgeVal(hsv.h, range.hue, true) &&
					judgeVal(hsv.s, range.satulation) &&
					judgeVal(hsv.v, range.value)
				) {
		...
		// cExtは冗長～不要
		function latLng2coverageImageXY(
			pixWidth,
			pixHeight,
			lng,
			lat,
			...
			let pixel = latLng2coverageImageRawXY(
				pixWidth,
				pixHeight,
				lng,
				lat,
		...
		function latLng2coverageImageRawXY(
			pixWidth,
			pixHeight,
			lng,
			lat,
			...
			if (!targetCoverage.geo2svg) {
				...
				px =
					(pixWidth * (lng - cExt.lngMin)) /
					(cExt.lngMax - cExt.lngMin);
				py =
					pixHeight -
					(pixHeight * (lat - cExt.latMin)) /
						(cExt.latMax - cExt.latMin);
		...
		// cExtは冗長～不要
		function coverageImageXY2LatLng(
			pixWidth,
			pixHeight,
			px,
			py,
			...
			if (!targetCoverage.geo2svg) {
				...
				lng =
					cExt.lngMin +
					((px + 0.5) * (cExt.lngMax - cExt.lngMin)) / pixWidth;
				lat =
					cExt.latMin +
					((pixHeight - py - 0.5) * (cExt.latMax - cExt.latMin)) /
						pixHeight;
				//			console.log("lat:",lat," lng:",lng,"  cExt:",cExt,"    px:",px,"  py:",py);
			} else {
				var s2g = targetCoverage.transform;
		...
		var polygonFilling = function (points, pixWidth, pixHeight) {
			...
			for (let y = minY; y <= maxY + 1; y++) {
				...
				for (let i = 1; i < meetPointAtOrigLine.length; i += 2) {
					if (
						Math.abs(
							meetPointAtOrigLine[i - 1] - meetPointAtOrigLine[i]
						) > 0.0001
					) {
				...
				for (let i = 1; i < meetPointAtCenterLine.length; i += 2) {
					if (
						Math.abs(
							meetPointAtCenterLine[i - 1] -
								meetPointAtCenterLine[i]
						) > 0.0001
					) {
						// 2点間の距離が極小の場合は面積なしとして扱う
						if (meetPointAtCenterLine[i] > 0) {
							...
							if (meetPointAtCenterLine[i - 1] < 0) {
								//始点がマイナスの場合は0で置き換え=クロッピング
								scanLine.unshift([
									[0, Math.floor(meetPointAtCenterLine[i])],
									y,
								]);
							} else {
								scanLine.unshift([
				...
				if (cAns.hasIntersection && cAns.hasOutOfRange) {
					...
					for (var j = 0; j < dsrc.length; j++) {
						if (
							i != j &&
							dsrc[j] &&
							isIntersect(cExtent, dsrc[j].extent)
						) {
							// 自身は対象外だね
					...
					for (var py = 0; py < cAns.height; py++) {
						for (var px = 0; px < cAns.width; px++) {
							...
							if (
								...
								var pLng =
									cAns.extent.lngMin +
									px * (cAns.extent.width / cAns.width);
								var pLat =
									cAns.extent.latMax -
									py * (cAns.extent.height / cAns.height);

								for (
									var k = 0;
									k < inersectedTiles.length;
									k++
								) {
									var iTile = inersectedTiles[k].tile;
									iPx = Math.floor(
										(iTile.width *
											(pLng - iTile.extent.lngMin)) /
											iTile.extent.width
									);
									iPy = Math.floor(
										(iTile.height *
											(iTile.extent.latMax - pLat)) /
											iTile.extent.height
									);
									...
									if (
										...
										if (iTile.rasterData[iPy][iPx].color) {
											...
											var comPix = computePixCoverage(
												...
												iTile.width,
												iTile.height,
												superParam.pixDataBuffer[
													inersectedTiles[k].index
												],
												superParam.range
											);
		...
		function doPolylineOrComputing(superParam) {
			...
			for (var i = 0; i < dsrc.length; i++) {
				...
				if (cAns.hasOutOfRange) {
					...
					var inersectedTiles = []; // そのタイルと重なっているタイルを探す
					for (var j = 0; j < dsrc.length; j++) {
						if (i != j && isIntersect(cExtent, dsrc[j].extent)) {
							// 自身は対象外だね
					...
					for (var j = 0; j < cAns.length; j++) {
						...
						if (tPoi.inRange == false) {
							...
							for (var k = 0; k < inersectedTiles.length; k++) {
								...
								if (
									inersectedTiles[k].extent.lngMin <=
										tPoi.coordinates[0] &&
									inersectedTiles[k].extent.lngMax >
										tPoi.coordinates[0] &&
									inersectedTiles[k].extent.latMin <=
										tPoi.coordinates[1] &&
									inersectedTiles[k].extent.latMax >
										tPoi.coordinates[1]
								) {
	...
	*/
		...
		function putPoint(
			...
			if (
				metastyle.styles.title != null &&
				metastyle.styles.title != undefined
			) {
		...
		function buildMetaDictHash(metaDictionary) {
		...
		function renderImages(
			...
			for (var tc = 0; tc < sourceDataArray.length; tc++) {
				...
				var tileImageUri = svgMapGIStool.getImage(
					tile,
					renderingOptions
				);
				...
				if (!tile.transform) {
					...
					rct.setAttribute(
						"width",
						Math.abs(tile.extent.width * crs.a)
					);
					rct.setAttribute(
						"height",
						Math.abs(tile.extent.height * crs.d)
					);
				} else {
