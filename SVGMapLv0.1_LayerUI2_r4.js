...
(function (window, undefined) {
	...
	**/
		...
		function setLayerTableStep2() {
			var tableHeight =
				document.getElementById("layerTable").offsetHeight;
			if (tableHeight == 0) {
			...
			if (tableHeight < layerListMaxHeight - layerListFoldedHeight - 2) {
				layerList.style.height =
					tableHeight + layerListFoldedHeight + 2 + "px";
		...
		function checkLayerListAndRegistLayerUI() {
			...
			for (var i = 0; i < layerProps.length; i++) {
				if (layerProps[i].visible) {
					...
					if (
						layerProps[i].svgImageProps &&
						layerProps[i].hasDocument
					) {
	...
	**/
		...
		function toggleBatch(e) {
			...
			for (var i = 0; i < batchLayers[lid].length; i++) {
				if (
					batchLayers[lid][i].getAttribute("visibility") == "hidden"
				) {
					bVisibility = "visible";
					break;
				}
		...
		function initLayerList(initOptions) {
			...
			if (layerList) {
				...
				var layersCustomizerPath =
					layerList.getAttribute("data-customizer");
		...
		function initLayerSpecificUI() {
			...
			if (layerSpecificUI) {
				...
				console.log(
					"can't find id:initLayerSpecificUI elem ... create it"
				); // 2020/12/01 うまく動いてない？
		...
		function assignLayerSpecificUiElement(
			targetElement,
			isInline,
			autoSizing
		) {
		...
		function getVisibleLayerSpecificUIid() {
			...
			for (
				var i = layerSpecificUIbody.childNodes.length - 1;
				i >= 0;
				i--
			) {
		...
		function showLayerSpecificUI(e) {
			...
			if (!e.target.hiddenOnLaunch) {
				if (
					!preDefinedTargetUi.element ||
					preDefinedTargetUi.isInline
				) {
		...
		function imgResize(img, parentDiv, size) {
			if (size.width != -1 && size.height != -1) {
				console.log(
					parentDiv.style.width + "/" + parentDiv.style.height
				);
				...
				if (img.width && img.height) {
					...
					layerSpecificUI.style.width =
						layerSpecificUiDefaultStyle.width;
					layerSpecificUI.style.height =
						layerSpecificUiDefaultStyle.height;
				}
			}
	...
	**/
		...
		function iframeOnLoadProcess(iframe, lid, reqSize, controllerURL, cbf) {
			...
			iframe.contentWindow.putGlobalMessage =
				putGlobalMessageForLayer(lid); // added 2019/12/05 今後、この種の"そのレイヤーに対するAPI"が増えると思うが、もう少しきれいにまとめたい。(TBD)
			...
			if (transferCustomEvent2iframe[lid]) {
				...
				transferCustomEvent2iframe[lid] =
					transferCustomEvent4layerUi(lid);
			}
			if (typeof iframe.contentWindow.preRenderFunction == "function") {
				...
				console.log(
					"Register preRenderControllerFunction for layerID:",
					lid
				);
		...
		function releaseLoadingFlag(layerId, sip) {
			if (sip[layerId].xhrLoading) {
				...
				console.error(
					"svgImagesProps[" +
						layerId +
						"].xhrLoading flag is inconsistent."
				);
			}
		...
		function testIframeSize(iframe, reqSize) {
			...
			var maxHeight =
				window.innerHeight -
				pxNumb(layerSpecificUiDefaultStyle.top) -
				50;
			var maxWidth =
				window.innerWidth -
