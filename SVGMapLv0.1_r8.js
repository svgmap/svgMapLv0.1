...
//  
//  Copyright (C) 2012-2013 by Satoru Takagi @ KDDI CORPORATION
//  
// License: (GPL v3)
...
//	console.log("mouse:"+mouseX0+","+mouseY0+" : geo["+mouseGeoPos.lat+","+mouseGeoPos.lng+"]");
	...
	timerID = setInterval( panningAnim , 50 ); // not use string param ( eval )
...
function setCssTransform( elem , tMat ){
	var tVal = "matrix3d(" + tMat.a + "," + tMat.b + ",0,0," + tMat.c + "," + tMat.d +",0,0,0,0,1,0," + tMat.e + "," + tMat.f + ",0,1)";
//	var tVal = "matrix(" + tMat.a + "," + tMat.b + "," + tMat.c + "," + tMat.d +"," + tMat.e + "," + tMat.f + ")";
...
//			console.log( "zoom : now:" , zoomingTransitionFactor , " target:" , 1/zoomFactor , " eTime:" , elapsedTime , " tTime:" , smoothZoomTransitionTime );
			...
			setTimeout( smoothZoom , 20 , zoomFactor , startDate , false , startZoom);
		} else {
//			console.log("to end zoom", 1/ zoomFactor);
			shiftMap( 0 , 0 , 1/ zoomFactor);
			setTimeout( smoothZoom , 20 , zoomFactor , startDate , true , startZoom); //フィニッシュ処理へ
		}
	} else { // フィニッシュ処理
		if ( additionalZoom != 0 ){
//			console.log("do additional Zoom2: ", zoomFactor * additionalZoom);
			setTimeout( smoothZoom , 20 , zoomFactor * additionalZoom , new Date() , false , zoomFactor );
			additionalZoom = 0;
		} else {
...
// 不要な機能な気がするのは機能性なのだろうか？ 2014/5/27確認中
...
// Firefox 28において、httpRes.responseやresponseTextはあるにも関わらず、responseXMLが取得できない場合に、(httpRes.responseXML != null)も評価しておかないと、データが表示されなくなる。responseXMLのみがnullの場合は、responseTextを利用して表示すればよい。
		if ((httpRes.responseXML != null) && httpRes.responseXML.documentElement && !isIE && verIE >= 100 ){
...
//					console.log(child2canvas,ip.transform,ip.x,ip.y );
	...
	//					console.log(symbols[symbId]);
						...
						imgBox.width = symb.width; // もともとゼロだったので・・
...
//					if ( docId == "root" ){			console.log ( ip.href , beforeElem);		}
					...
					if ( childCategory == POI || childCategory == BITIMAGE ){ // image,use要素の場合
						...
						img = getImgElement(xd.p0 , yd.p0, xd.span , yd.span , imageURL , imageId , ip.opacity , childCategory , ip.metadata , ip.title , elmTransform );
...
//						console.log("load:" + imageId);
						...
						/**
						if( ip.href.indexOf("Cntr0029_l4_28-83.svg") >=0){
							// 伊豆の松崎町のちょっと下　バグが出るタイル
							console.log("問題のタイルを読み込む id:" +  imageId );
							console.log( img );
						}
						**/
...
//						console.log("call loadSVG:",imageId, ip.href);
						loadSVG( docDir + ip.href , imageId , img , docId);
...
//						console.log("AlreadyLoadedBitimage:" + imageId + " dispay:" + imgElem.style.display);
						// x,y,w,hを書き換える
						setImgElement(imgElem , xd.p0 , yd.p0, xd.span , yd.span , getImageURL(ip.href,docDir), elmTransform );
...
//						console.log("rootLayer:",svgImagesProps[docId].rootLayer);
						document.getElementById(svgImagesProps[docId].rootLayer).appendChild(inCanvas); //前後関係をもう少し改善できると思う
						inCanvas.setAttribute("hasdrawing","false");
	...
	for ( var i = parentNode.childNodes.length - 1 ; i >= 0 ; i -- ){
		...
		if ( oneNode.nodeType == 1 ){
			if ( oneNode.nodeName != "DIV" ){
				...
				if ( ans ){
//					console.log("remove span:",oneNode);
					oneNode.parentNode.removeChild(oneNode);
				} else {
					ret = false;
				}
...
function getImgElement( x, y, width, height, href , id , opacity , category , meta , title , transform ){
...
function setImgElement( img , x, y, width, height, href , transform , cdx , cdy , txtFlg , txtNonScaling ){
	if ( ! cdx ){
		cdx = 0;
	}
	if ( ! cdy ){
...
//	img.style.position = "absolute";
	...
	if ( !txtFlg && img.src && href && img.src != href){ // firefoxでは(同じURLかどうかに関わらず)srcを書き換えるとロードしなおしてしまうのを抑制 2014.6.12
		img.src = href;
	}
	if ( transform ){ // ま、とりあえず 2014.6.18
		img.style.transform = "matrix(" + transform.a + ","  + transform.b + "," + transform.c + "," + transform.d + "," + transform.e + "," + transform.f + ")";
...
function getImageProps( imgE , category , parentProps , subCategory ){
	var x, y, width, height, meta, title, elemClass, href, transform, text , cdx , cdy;
...
//		console.log("centralGeo:", cent.lat , cent.lng);
	...
	if ( vScale ){ // 50pxのたてスケールに相当する長さをKmで表示
		vScale.innerHTML = rount(getVerticalScreenScale( 50 ), 3 ) + "Km";
	}
}
...
//		var batchLayers = getSwLayers(  );
		var bsize = 0;
		for (var j in batchLayers) { bsize++; }
		
		
//		console.log("batchLayers:",batchLayers , "size:" , bsize);
...
//		console.log("layerCount:" + layerUI.length );
		var lcount;
		if ( !layerUImulti ){
			layerUI.length = layers.length + 1 + bsize;
			layerUI.options[0].innerHTML = "=LAYER=";
			lcount = 1;
		} else {
			layerUI.length = layers.length + bsize;
			lcount = 0;
		}
		for ( var i = layers.length - 1  ; i >= 0 ; i-- ){
			var sel = false;
			var batchSetName ="";
			if ( layers[i].getAttribute("class") && layers[i].getAttribute("class").split(" ").indexOf("batch")!=-1){
				// バッチレイヤー
//				console.log("found batch:",layers[i].getAttribute("class"));
				var blName = removeLayerCatName( layers[i].getAttribute("class").split(" ") , "switch" , "batch" , "editable" , "clickable" )[0];
//				console.log("name:",blName);
				batchSetName = blName + "/";
				if ( ! batchLayers[blName].listed ){
					layerUI.options[lcount].innerHTML = blName + "/*";
					layerUI.options[lcount].value = "batch " +blName;
					
					var blStyle ="color:#2020FF";
					sel = true;
					var batchLayers = getSwLayers( "batch" ); 
					for ( var ii = 0 ; ii < batchLayers[blName].length ; ii++){
						if ( (batchLayers[blName])[ii].getAttribute("visibility" ) == "hidden"){
					...
					if (!isIE || !layerUImulti ){ // IEでmultipleの場合とにかくバグがひどい
						layerUI.options[lcount].setAttribute("style" , blStyle);
					}
					batchLayers[blName].listed = true;
					if ( sel ){
						layerUI.options[lcount].selected = true;
					} else {
						layerUI.options[lcount].selected = false;
					}
			...
			var optText = batchSetName;
			if ( layers[i].getAttribute("title")){
				optText += layers[i].getAttribute("title");
			} else {
				optText += layers[i].getAttribute("xlink:href");
				if ( ! optText ){
					optText += layers[i].getAttribute("src");
				}
			}
...
//			console.log("isEditable?"+isEditableLayer(layers[i]),layers[i]);
			...
			layerUI.options[lcount].innerHTML = optText;
			
			if (!isIE || !layerUImulti ){ // IEでmultipleの場合とにかくバグがひどい 2014.09.04
				layerUI.options[lcount].setAttribute("style" , style);
			}
			layerUI.options[lcount].value = layers[i].getAttribute("iid");
			if ( sel ){
				layerUI.options[lcount].selected = true;
			} else {
				layerUI.options[lcount].selected = false;
...
function testClick( obj ){ // html:img要素によるPOI(from use要素)に設置するクリックイベント
//	console.log("testClick",obj);
	var target = obj.target || obj.srcElement ; 
...
//		pvs.style.height="80%";
		...
		pvs.innerHTML='<input type="button" id="pvsView" value="属性を見る"/><br><input type="button" id="pvsLink" value="リンクに移動する"/>';
...
//		pes.style.height="80%";
		...
		pes.innerHTML='<input type="button" id="pesView" value="属性を見る"/><br><input type="button" id="pesEdit" value="編集する"/><br><input type="button" id="pesSer" value="シリアライズ"/><br><input type="button" id="pesDel" value="削除する"/><br><input type="button" id="pesClose" value="閉じる"/>';
...
var styleCatalog = new Array("stroke" , "stroke-width" , "stroke-linejoin" , "stroke-linecap" , "fill" , "fill-rule" , "fill-opacity" , "opacity" , "vector-effect" , "display" , "font-size"); 
...
//		console.log(i);
		if ( (svgImagesProps[i].Path).indexOf( docPath )>=0 ){
			ans= i;
//			console.log("found!");
			break;
		}
