...
(function (window, undefined) {
	...
	var svgMapAuthoringTool = (function () {
		...
		function setTools(e) {
			console.log(
				"get iframe appear event from authoring tools framework."
			);
			...
			targetDoc.getElementById("editConf").addEventListener(
				...
				function (e) {
					...
					if (uiMapping.modifyTargetElement) {
						uiMapping.prevAttrs = getAllAttrs(
							uiMapping.modifyTargetElement
						);
					}
					var ret = null;
						...
						case "pepok": // 値設定決定用
							...
							if (uiMapping.editingMode === "POI") {
								...
								if (
									uiMapping.modifyTargetElement &&
									document.getElementById(
										uiMapping.modifyTargetElement.getAttribute(
											"iid"
										)
									)
								) {
				...
				**/
								...
								console.log(
									"remove a point not skip edit conf"
								);
								confStat = null;
		...
		function readPoiUiParams(targetDoc) {
			...
			for (var i = 0; i < tbl.rows.length; i++) {
				...
				if (tbl.rows[i].cells[1].childNodes[0].dataset.type) {
					if (
						tbl.rows[i].cells[1].childNodes[0].dataset.type ==
						"latMetaCol"
					) {
			...
			targetDiv.addEventListener(
				...
				function (e) {
					...
					if (e.target.parentNode.id == "pointUI") {
						...
						for (
							var i = 0;
							i < e.target.parentNode.childNodes.length;
							i++
						) {
		...
		function setTargetObject(svgTarget) {
			...
			if (
				...
				uiMapping.editingLayerId ===
					svgImagesProps[svgTarget.docId].rootLayer
			) {
				// 冗長・・
				...
				if (
					svgNode.nodeName == "use" &&
					uiMapping.editingMode == "POI"
				) {
		...
		function displayPOIprops(svgTarget) {
			...
			uiDoc.getElementById("symbol" + props.href).style.borderColor =
				"red";
