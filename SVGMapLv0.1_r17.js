	...
	var svgMap = (function () {
		...
		function initLoad() {
			...
			if (mapCanvas.dataset.src) {
				...
				console.log(
					"NO id:mapcanvas data-src for root svg container exit.."
				);
				return;
			}
			...
			if (panning) {
				...
				if (zoomingTransitionFactor > 0) {
					if (initialTouchDisance == 0) {
						zoomingTransitionFactor =
							Math.exp(-difY / (mapCanvasSize.height / 2)) /
							Math.exp(0);
					}
				...
				// リミッターかけてみたけど意味ないかな・・
				if (
					Math.abs(prevX - difX) > 200 ||
					Math.abs(prevY - difY) > 200
				) {
					endPan();
				} else {
					prevX = difX;
	...
	**/
		...
		function tempolaryZoomPanImages(zoomFactor, sftX, sftY) {
			...
			if (uaProp.IE) {
				...
				for (var i = mapImgs.length - 1; i >= 0; i--) {
					var il = Number(
						mapImgs.item(i).style.left.replace("px", "")
					);
					var it = Number(
						mapImgs.item(i).style.top.replace("px", "")
					);
	...
	//			++ removedImgs;
		...
		function loadSVG(path, id, parentElem, parentSvgDocId) {
			...
			if (!svgImages[id]) {
				...
				if (httpObj) {
					...
					const tempToken =
						svgImagesProps[id].specialToken ||
						svgImagesProps[svgImagesProps[id].rootLayer]
							?.specialToken; // ?. : オプショナルチェーンという書き方でアクセス元がNullでも途中でエラーとならない書き方
					if (tempToken != undefined) {
						// クエリストリングを用いた認可を突破する用
						rPath = addSpecialTokenAtQueryString(rPath, tempToken);
						svgImagesProps[id].specialToken = tempToken; //Rootのプロパティ
					}

					if (
						typeof contentProxyParams.getUrlViaProxy == "function"
					) {
		...
		function repairScript(resTxt) {
			...
			resScript = resScript.replace(/</g, "&lt;");
			resScript =
				"<script>" + resScript.replace(/>/g, "&gt;") + "</script>";
		...
		function handleResult(
			...
			**/
				...
				if (
					...
					if (
						...
						if (resTxt.match(/data-controller-src([\s\S]*)"/)) {
							resTxt = getControllerSrc(
								resTxt,
								svgImagesProps[docId]
							);
						}
				...
				svgImages[docId].getElementById =
					getElementByIdUsingQuerySelector; // added 2017.2.3
				...
				if (
					...
					svgImagesProps[parentSvgDocId].childImages[docId] ==
						CLICKABLE
				) {
				...
				if (svgImagesProps[docId].script) {
					...
					svgImagesProps[docId].script.scale =
						zoom * child2root.scale; // patch 2018.5.18 なんか汚い・・・
					svgImagesProps[docId].script.CRS =
						svgImagesProps[docId].CRS;
		...
		function dynamicLoad(docId, parentElem) {
			...
			if (docId == "root") {
				...
				if (typeof setLayerUI == "function") {
					...
					setLayerUI({
						updateLayerListUITiming: "setRootLayersProps",
		...
		function handlePreRenderControllerScript(
			...
			if (isSuperController) {
				...
				try {
					...
					svgImagesProps[docId].preRenderControllerFunction(
						svgDocStatus
					);
				} catch (e) {
...
**/
		...
		function parseSVG(
			...
			if (svgElem.nodeName == "svg") {
				...
				if (
					typeof svgImagesProps[docId].preRenderControllerFunction ==
					"function"
				) {
					// 2020/6/8  svg要素内のscriptをevalで実行するのをやめる前準備
				...
				if (typeof preRenderSuperControllerFunction == "function") {
					handlePreRenderControllerScript(
						docId,
						zoom,
						child2root,
						true
			...
			for (var i = 0; i < svgNodes.length; i++) {
				...
				if (
					...
					if (ip.specialToken) {
						if (docId != "root") {
							// specialTokenを保存するのはroot以外が対象
							svgImagesProps[
								svgImagesProps[docId].rootLayer
							].specialToken = ip.specialToken;
						}
					}
						...
						if (!imgElem) {
							...
							if (
								...
								if (!svgImagesProps[docId].childImages) {
									svgImagesProps[docId].childImages =
										new Array();
								}
								if (
									svgImagesProps[docId].isClickable ||
									(ip.elemClass &&
										ip.elemClass.indexOf("clickable") >= 0)
								) {
									svgImagesProps[docId].childImages[imageId] =
										CLICKABLE;
								} else {
									svgImagesProps[docId].childImages[imageId] =
										EXIST;
							...
							if (
								...
								if (beforeElem) {
									...
									if (parentElem.hasChildNodes()) {
										...
										var childSpans =
											parentElem.getElementsByTagName(
												"div"
											);
										if (childSpans) {
											parentElem.insertBefore(
							...
							if (
								...
								if (
									...
									console.log(
										"change SVG's path. this has issues...."
									);
		...
		function checkDeletedNodes(parentNode) {
			...
			for (var i = parentNode.childNodes.length - 1; i >= 0; i--) {
				...
				if (oneNode.nodeType == 1) {
					...
					if (
						(oneNode.nodeName == "IMG" ||
							oneNode.nodeName == "SPAN") &&
						oneNode.id &&
					...
					for (var k = 0; k < symbolNodes.length; k++) {
						if (symbolNodes[k].nodeName == "image") {
							...
							if (symbolNodes[k].hasChildNodes) {
								for (
									var l = 0;
									l < symbolNodes[k].childNodes.length;
									l++
								) {
									if (
										symbolNodes[k].childNodes[l].nodeType !=
										1
									) {
										continue;
									} else if (
										symbolNodes[k].childNodes[l].nodeName ==
										"image"
									) {
										var symb = getSymbolProps(
											symbolNodes[k].childNodes[l]
										);
										if (!symb.id) {
											symb.id =
												symbolNodes[k].getAttribute(
													"id"
												);
										}
										symbols["#" + symb.id] = symb;
										...
										var symb = getGraphicsGroupSymbol(
											symbolNodes[k]
										);
										symbols["#" + symb.id] = symb;
										break;
		...
		function getrootViewBoxFromRootSVG(
			...
			if (viewBox) {
				if (
					...
					rVPheight =
						(viewBox.width * mapCanvasSize_.height) /
						mapCanvasSize_.width;
					rVPx = viewBox.x;
					rVPy = viewBox.y + viewBox.height / 2.0 - rVPheight / 2.0;
				} else {
					rVPheight = viewBox.height;
					rVPwidth =
						(viewBox.height * mapCanvasSize_.width) /
						mapCanvasSize_.height;
					rVPy = viewBox.y;
		...
		function mercator() {
			...
			function MercatorXY2latLng(px, py) {
				...
				var lat =
					90 -
					(360 * Math.atan(Math.exp(-y * 2 * Math.PI))) / Math.PI;
				var lng = 360 * x;
		...
		function getImgElement(
			...
			if (typeof contentProxyParams.getUrlViaImageProxy == "function") {
				...
				href =
					contentProxyParams.getNonlinearTransformationProxyUrl(href);
	...
	**/
		...
		**/
				if (globalView && globalView.nodeName == "view") {
					...
					if (gcsElem) {
						...
						**/
								...
								var tFunc =
									svgImagesProps[docId].script.getFunction(
										tf
									);
								if (tFunc) {
									genericCRS = tFunc();
		...
		function setController(svgDoc, docPath, svgImageProps) {
			...
			var cntPath =
				svgDoc.documentElement.getAttribute("data-controller");
		...
		function getConversionMatrixViaGCS(fromCrs, toCrs) {
			...
			if (toCrs.transform || fromCrs.transform) {
				...
				if (ifCrs.inverse) {
					...
					sif = Math.sqrt(
						Math.abs(ifCrs.a * ifCrs.d - ifCrs.b * ifCrs.c)
					);
				}
				if (toCrs.inverse) {
					st = toCrs.scale;
					...
					st = Math.sqrt(
						Math.abs(toCrs.a * toCrs.d - toCrs.b * toCrs.c)
					);
				}
				/**
		if ( ifCrs.inverse ){
				...
				**/
				...
				if (
					...
					((imgE.getAttribute("style") &&
						imgE
							.getAttribute("style")
							.indexOf("image-rendering:pixelated") >= 0) ||
						(parentProps &&
...
//				console.log("use Microsoft.XMLHTTP");
		...
		**/
		...
		function removeChildDocs(imageId) {
			...
			if (svgImages[imageId]) {
				...
				svgImagesProps[imageId] &&
				svgImagesProps[imageId].loadError
			) {
				delete svgImagesProps[imageId];
			}
		}
		...
		function setLayerListSize() {
			...
			llElemSize = Number(
				llElemSize.substring(0, llElemSize.indexOf("px"))
			);
	...
	**/
		...
		function updateCenterPos() {
			if (centerPos) {
				...
				centerPos.innerHTML =
					round(cent.lat, 6) + " , " + round(cent.lng, 6);
			}
			if (vScale) {
			...
			if (gpsb) {
				if (navigator.geolocation) {
					...
					if (
						...
						console.log(
							"contentWindow:",
							gpsif.contentWindow.document
						);
						var ifd = gpsif.contentWindow.document.documentElement;
						var script =
							gpsif.contentWindow.document.createElement(
								"script"
							);
		...
		**/
				...
				if (!layer) {
					...
					for (var i = 0; i < layers.length; i++) {
						if (
							...
							layers[i].getAttribute("xlink:href") ==
							layerID_Numb_Title
						) {
							layer = layers[i];
							break;
		...
		function isEditingLayer(layer) {
			...
			if (layer) {
				...
				if (
					svgImagesProps[layerId] &&
					svgImagesProps[layerId].editing
				) {
					return true;
				} else {
					return false;
		...
		function checkLayerSwitch(selectedLayer) {
			...
			if (selectedLayer.getAttribute("class")) {
				selectedLayerClass = selectedLayer
					.getAttribute("class")
					.split(" ");
		...
		function getElementsByDualTagName(doc, tagn1, tagn2) {
			...
			if (verIE > 8) {
				layers = Array.prototype.slice.call(
					doc.getElementsByTagName(tagn1)
				);
			...
			if (editing && lp.editing != editing) {
				...
				for (var i = 0; i < rootLayersProps.length; i++) {
					...
					if (svgImagesProps[rootLayersProps[i].id]) {
						if (
							svgImagesProps[rootLayersProps[i].id].editing ==
							true
						) {
							svgImagesProps[rootLayersProps[i].id].editing ==
								false;
	...
	**/
		...
		function getTickerMetadata() {
			for (var i = 0; i < tickerTableMetadata.length; i++) {
				...
				if (tm.img) {
					...
					tm.metaSchema =
						svgTarget.ownerDocument.firstChild.getAttribute(
							"property"
						);
				} else {
					// 2D Vector
					...
					for (var i = 0; i < hittedObjects.elements.length; i++) {
						...
						var vcbf = (function (elem, parent, bbox) {
							return function () {
								...
								vectorDataWrapperForShowPoiProperty(
									elem,
									bbox,
									parent
								);
						...
						tickerTableMetadata.push({
							...
							geoBbox: {
								...
								width:
									vMeta.geolocMax.lng - vMeta.geolocMin.lng,
								height:
									vMeta.geolocMax.lat - vMeta.geolocMin.lat,
							},
		...
		function addTickerItem(title, callBack, table, subTitle) {
			...
			if (subTitle) {
				spn.innerHTML =
					title + "<font size='-2'>/" + subTitle + "</font>";
			} else {
				spn.innerHTML = title;
			}
			td.appendChild(spn);
		...
		function setImgViewport(target, href_fragment) {
			...
			var clipX =
				parseFloat(target.style.left) - iScaleX * Number(imgBox[0]);
			var clipY =
				parseFloat(target.style.top) - iScaleY * Number(imgBox[1]);
			...
			for (var riy = 0; riy < cih; riy++) {
				...
				for (var rix = 0; rix < ciw; rix++) {
					...
					if (
						...
						var saddr =
							(Math.floor(cixy.x) + Math.floor(cixy.y) * ciw) * 4;
						...
						if (prevColHasData) {
							...
							dstData.data[daddr + 1] =
								dstData.data[daddr - 4 + 1]; // g
							dstData.data[daddr + 2] =
								dstData.data[daddr - 4 + 2]; // b
							dstData.data[daddr + 3] =
								dstData.data[daddr - 4 + 3]; // a
						} else if (prevRowHasData[rix]) {
							// サブピクセルオーダーの継ぎ目を消す処理(Y方向)
							// y方向ひとつ前のピクセルに値があればその値をコピーする
							dstData.data[daddr] = dstData.data[daddr - prow]; // r
							dstData.data[daddr + 1] =
								dstData.data[daddr - prow + 1]; // g
							dstData.data[daddr + 2] =
								dstData.data[daddr - prow + 2]; // b
							dstData.data[daddr + 3] =
								dstData.data[daddr - prow + 3]; // a
						}
...
**/
		...
		function getDocumentId(svgelement) {
			...
			return svgelement.ownerDocument.documentElement.getAttribute(
				"about"
			);
		}
		...
		function poiSelectProcess(obj) {
			...
			if (
				...
				el &&
				el.getAttribute("iid") ==
					svgImagesProps[target.parentNode.getAttribute("id")]
						.rootLayer
			) {
			...
			cm.addEventListener(
				...
				function (e) {
					...
					if (e.target.id.indexOf("customModalBtn_") >= 0) {
						...
						if (callback) {
							callback(
								Number(e.target.id.substring(15)),
								callbackParam
							);
						}
		...
		function setSVGpathPoints(
			...
			function getHitPoint(hp, cp, isEdgePoint) {
				...
				if (hp.prevX) {
					...
					if (
						...
						(isEdgePoint || hp.prevIsEdgePoint)
					) {
				...
				if (
					...
					if (!hp.x) {
						...
						!isEdgePoint &&
						(hp.isEdgePoint || hp.isNearEdgePoint)
					) {
				...
				switch (command) {
					...
					case "L":
						...
						if (GISgeometry && !vectorEffectOffset) {
							var svgP = [sx, sy];
							var thisPs =
								GISgeometry.svgXY[GISgeometry.svgXY.length - 1];
							thisPs.push(svgP);
						}
						break;
					case "l":
			...
			var vabs = Math.sqrt(
				(endX - prevX) * (endX - prevX) +
					(endY - prevY) * (endY - prevY)
			);
			if (vabs) {
		...
		function getStyle(svgNode, defaultStyle, hasHyperLink) {
			...
			if (svgNode.getAttribute("visibleMinZoom")) {
				style.minZoom =
					Number(svgNode.getAttribute("visibleMinZoom")) / 100.0;
			...
			if (svgNode.getAttribute("visibleMaxZoom")) {
				style.maxZoom =
					Number(svgNode.getAttribute("visibleMaxZoom")) / 100.0;
		...
		function setCanvasStyle(style, context) {
			...
			if (style) {
				...
				if (style["stroke-dasharray"]) {
					...
					var dashList =
						style["stroke-dasharray"].split(/\s*[\s,]\s*/);
				...
				for (var i in visiblePOIs) {
					if (i != POIelemId) {
						if (
							...
							targetPOI.x >
								visiblePOIs[i].x + visiblePOIs[i].width ||
							targetPOI.y + targetPOI.height < visiblePOIs[i].y ||
							targetPOI.y >
								visiblePOIs[i].y + visiblePOIs[i].height
						) {
							// none
						} else {
							visiblePOIs[i].id = i;
						...
						for (var i = 0; i < group.length; i++) {
							if (lvl % 2 == 0) {
								...
								boundaries.push({
									id: group[i].id,
									position:
										group[i].obj.x + group[i].obj.width,
									open: false,
								});
							} else {
								boundaries.push({
								...
								boundaries.push({
									id: group[i].id,
									position:
										group[i].obj.y + group[i].obj.height,
									open: false,
								});
							}
						}
		...
		function getHyperLink(svgNode) {
			...
			while (oneNode.parentNode) {
				...
				if (
					oneNode.nodeName == "a" &&
					oneNode.getAttribute("xlink:href")
				) {
					return {
				...
				if (typeof hitTest == "function") {
					...
					if (hitted) {
						...
						for (var hi of hitted) {
							...
							if (hi === true) {
								ans.element =
									svgImages[layerId].documentElement; // Elementが必要なので文書要素を・・
								ans.title = layerName;
								ans.metadata = hindex;
							} else if (typeof hi == "string") {
								ans.element =
									svgImages[layerId].documentElement; // 同上
								ans.title = hi;
								ans.metadata = hi;
		...
		function getVectorMetadata(element, parent, bbox) {
			...
			if (element.ownerDocument.firstChild.getAttribute("property")) {
				metaSchema =
					element.ownerDocument.firstChild.getAttribute("property");
			}
			return {
				geolocMin: geolocMin,
				geolocMax: geolocMax,
		...
		function checkAndKickEditor(hittedVectorObjects, x, y) {
			...
			if (typeof svgMapAuthoringTool == "object" && el) {
				...
				if (
					hittedVectorObjects &&
					hittedVectorObjects.elements.length > 0
				) {
					// ヒットしている場合
	...
	**/
			...
			targetElement.setAttribute(
				"lat",
				geolocMin.lat + "," + geolocMax.lat
			);
			targetElement.setAttribute(
				"lng",
				geolocMin.lng + "," + geolocMax.lng
			);
	...
	**/
		...
		function showUseProperty(target) {
			...
			var title = document.getElementById(
				target.getAttribute("iid")
			).title; // Added title 2017.8.22
		...
		function defaultShowPoiProperty(target) {
			...
			if (target.getAttribute("content")) {
				...
				var metaData = parseEscapedCsvLine(
					target.getAttribute("content")
				);

				message +=
					"<tr><th style='width=25%'>name</th><th>value</th></tr>";
				if (titleAndLayerName != "") {
					message +=
						"<tr><td>title/Layer</td><td> " +
						titleAndLayerName +
						"</td></tr>";
				}
		...
		function contColorSetContinuous(interval) {
			var csMode;
			if (
				document.getElementById("contButton").innerHTML ==
				"contourSearch[m]"
			) {
				document.getElementById("contButton").innerHTML =
					"disableSearch";
		...
		function setCookie(key, value, expire) {
			...
			var rootPath = new URL(svgImagesProps["root"].Path, location.href)
				.pathname;
			window.localStorage[
				localStorageSvgMapSuffix + rootPath + "#" + key
			] = value;
		}
		...
		async function sha256(str) {
			...
			const buff = new Uint8Array(
				[].map.call(str, (c) => c.charCodeAt(0))
			).buffer;
			const digest = await crypto.subtle.digest("SHA-256", buff);
			// (from: https://stackoverflow.com/a/40031979)
			return [].map
				.call(new Uint8Array(digest), (x) =>
			...
			if (resumeFirstTime) {
				...
				if (cook.customLayers && window.svgMapCustomLayersManager) {
					try {
						...
						svgMapCustomLayersManager.applyCustomLayers(
							customLayers
						);
						parseSVG(
							documentElement,
				...
				if (cook.customGeoViewboxes) {
					...
					var customGeoViewboxes = JSON.parse(
						cook.customGeoViewboxes
					);
					console.log("customGeoViewboxes:", customGeoViewboxes);
					if (customGeoViewboxes.currentSettingKey) {
						var cvb =
							customGeoViewboxes.settings[
								customGeoViewboxes.currentSettingKey
							];
						if (cvb) {
							setGeoViewPort(
								cvb.y,
		...
		**/
		...
		function getFragmentView(URLfragment) {
			...
			if (
				...
				var vals = URLfragment.substring(
					URLfragment.indexOf("viewBox")
				);
		...
		function captureGISgeometries(
			...
			if (GISgeometriesCaptureFlag) {
				// 2019/12/24 排他制御
				console.log(
					"Now processing another captureGISgeometries. Try later."
				);
				cbFunc(false);
				return false;
			}
							...
							if (
								...
								for (var j = 0; j < geom.svgXY.length; j++) {
									...
									if (
										...
										if (
											geom.type === "Polygon" &&
											(startP.lat != geoCrd.lat ||
												startP.lng != geoCrd.lng)
										) {
											wgSubP.push([
												startP.lng,
												startP.lat,
		...
		function setProxyURLFactory(
			...
			if (typeof imageURLviaProxyFunction == "function") {
				contentProxyParams.getUrlViaImageProxy =
					imageURLviaProxyFunction;
			...
			if (
				typeof imageURLviaProxyFunctionForNonlinearTransformation ==
				"function"
			) {
			...
			if (alsoCrossoriginParam) {
				...
				if (
					...
					return {
						...
						crossorigin:
							contentProxyParams.crossOriginAnonymousNonlinearTF,
					};
				} else {
					return {
						url: originalURL,
		...
		return {
			...
			captureGISgeometriesOption: function (
				...
				if (
					TreatRectAsPolygonFlg === true ||
					TreatRectAsPolygonFlg === false
				) {
					GISgeometriesCaptureOptions.TreatRectAsPolygonFlag =
			...
			setPreRenderController: function (layerId, pcf) {
				...
				if (typeof pcf == "function") {
					if (layerId) {
						svgImagesProps[layerId].preRenderControllerFunction =
							pcf;
					} else {
					...
					if (layerId) {
						delete svgImagesProps[layerId]
							.preRenderControllerFunction;
					} else {
